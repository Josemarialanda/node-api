...
loginUser
  :: ClientEnv -> Credentials -> IO (Either ClientError (Id User, Token))
  ...
  describe "Tagger" $ do
    describe "register user" $ do
      it "should register a user" $ \port -> do
        response <-
          registerUser (clientEnv port) (Credentials "marcosh" (Password "password"))
      ...
      it "should not register two users with the same name" $ \port -> do
        ...
        response <-
          registerUser (clientEnv port) (Credentials "marcosh" (Password "password1"))
        response `shouldSatisfy` hasStatus forbidden403

      it "should register two users with different names" $ \port -> do
        _ <- registerUser (clientEnv port) (Credentials "marcosh" (Password "password"))
        response <-
          registerUser (clientEnv port) (Credentials "perons" (Password "password"))
        response `shouldSatisfy` isRight
    ...
    describe "getContents" $ do
      it "retrieves all contents added by a user" $ \port -> do
        ...
        case contents of
          ...
          Right ownedContent ->
            ownedContent `shouldMatchList` [Owned userId content1, Owned userId content2]
      ...
      it "retrieves only contents from the requesting user" $ \port -> do
        ...
        case contents of
          Left _ -> fail "unable to retrieve contents"
          Right ownedContent ->
            ownedContent `shouldMatchList` [Owned userId1 content1, Owned userId1 content2]
