module Infrastructure.Database.Queries
  ( addContentWithTags
  , addUser
  , updatePasswordById
  , updateUsernameById
...
import qualified Core.Types.Profile                    as Profile
...
import           Hasql.Transaction.Sessions            (IsolationLevel (Serializable), Mode (Write),
                                                        transaction)

import           Infrastructure.Database.Schema        (contentSchema, contentsTagsSchema,
...
import           Infrastructure.Types.Database.Schema  (Content (..), ContentsTags (..), Profile,
                                                        ...
                                                        profileLastName, profileSex, profileUserId,
                                                        userId)
...
-- |
-- Given a 'Core.User' 'Id', retrieves all the contents for that specific user
selectUserContents
  :: Id Core.User -> Session [(Content Result, [Tag Result], User Result)]
...
-- If in the database we find none or more the one, it returns the appropriate error message
selectUserById :: Id Core.User -> Session (Either WrongNumberOfResults (User Result))
...
updatePasswordById :: Id Core.User -> EncryptedPassword -> Session ()
...
-- If the 'User' does not exist, it returns the appropriate error message
updateUsernameById :: Id Core.User -> Text -> Session ()
...
deleteUserById :: Id Core.User -> Session ()
...
getUserProfile :: Id Core.User -> Session (Either WrongNumberOfResults (Profile Result))
...
updateUserProfile :: Id Core.User -> Profile.Profile -> Session ()
